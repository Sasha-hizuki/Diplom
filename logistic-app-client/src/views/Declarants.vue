...
import Declarant from '@/components/Declarant'
export default {
    data(){
        return {
            ItemsList:[],
    ...
    methods:{
        async removeItem(id){
            await fetch('https://localhost:5001/api/declarants/'+id, { 
                method: 'DELETE',
                headers: {
        ...
        async onSubmit(){
            if(this.item.id){
                await fetch('https://localhost:5001/api/declarants/'+this.item.id, 
                { 
                    method: 'PUT',
